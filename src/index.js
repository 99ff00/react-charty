(function (global, factory) {
  typeof exports === "object" && typeof module !== "undefined" ? module.exports = factory(require("react"), require("prop-types")) :
      ...
      (global.Charty = factory(global.React, global.PropTypes));
}(this, (function (React, PropTypes) {
  "use strict";

  const {applyOpacityToColor, minFromArray} = require("./utils");
  ...
  var Charty = function () {
    ...
    function Charty(ID_, props, parent, UI_, ctx_) {
      ...
      var V = {progress: 0, needMeasure: true, yPos: []},
          ctx = ctx_,
          IDs = [],
          AY = [],
          AYL,
      ...
      function updateTheme(theme_) {
        ...
        if (V.zoomedChart) V.zoomedChart.getSeries().map(function (S, i, series) {
          S.color = theme.series && theme.series[S.name] ? theme.series[S.name] : S._color;
        });
      ...
      function start(restart) {
        ...
        Object.keys(data).forEach(function (n) {
          ...
          if (n === "x") AX = d; else AY.push({
            ...
            backgroundColor: (props.backgroundColors || {})[n],
            name: (props.names || {})[n],
            type: props.type
          });
        });
      ...
      function renderLinear(type, height, vStart, hPadding, offsetY, offsetX, startIdx, endIdx, scaleX, scaleY) {
        ...
        for (let sequence = 0, idx, x, data, color; sequence < AYL; sequence++) {
          ...
          const backgroundColor = AY[sequence].backgroundColor;
          ...
          let minY = minFromArray(yData);

          ctx.stroke();

          // Draw last point
          if (AX.length > 0 && backgroundColor) {
            ctx.lineTo(x, minY);
            x = offsetX + hPadding + (AX[0] - vStart) * scaleX;
            ctx.lineTo(x, minY);
            ctx.fillStyle = applyOpacityToColor(color, 0.4);
            ctx.fill();
          }
      ...
      function gradient(fromColor, toColor, minY, maxY) {
        let gradient = ctx.createLinearGradient(0, 0, 0, maxY);
      ...
      function toggleLegend(on) {
        ...
        if (on) {
          ...
          } else UI.xval.stylo({display: "none"});
      ...
      function updateCursor(area) {
        if (!IS_MOBILE && !STATE.draggingArea) {
          if (area === AREA.BRUSH_LEFT || area === AREA.BRUSH_RIGHT) UI.canvas.stylo({cursor: "ew-resize"}); else if (area === AREA.BRUSH_CENTER) UI.canvas.stylo({cursor: "move"}); else resetCursor();
        }
      }
        ...
        UI.canvas.on("touchmove mousemove", function (e, x, y, area) {
          for (var c = 0, cl = CHARTS.length; c < cl; c++) {
            if (c !== myIdx) {
              ...
              if (chart.dismiss && chart.dismiss === typeof "function") {
                CHARTS[c].dismiss();
              }
            }
          }
  ...
  var Charty$1 = function (_Component) {
    ...
    createClass(Charty$$1, [{
      ...
      value: function render() {
        return React__default.createElement("div", {ref: this.setRef, style: this.props.style});
      }
    }]);
    return Charty$$1;
  }(React.Component);
